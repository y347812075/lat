...
int latcd_send_request(int socket_fd, int source_fd,
...
int latcd_receive_request(int socket_fd, LatcdRequestV1 *request,
                          int *source_fd, char *error, size_t error_size)
{
    if (socket_fd < 0 || !request || !source_fd) {
    ...
    *source_fd = -1;
    ...
    if (!request_header_valid(request) || descriptor_count != 1) {
        for (size_t i = 0; i < descriptor_count; i++) {
            close(descriptors[i]);
        }
        return fail(error, error_size,
                    descriptor_count == 1 ? "invalid latcd request header" :
                    "latcd request must carry exactly one descriptor");
    }
